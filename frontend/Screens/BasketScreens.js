...
function BasketScreens({ navigation, productId, takeToken }) {
  ...
  useEffect(() => {
    const findBuyer = async () => {
      const data = await fetch(`${IP_HOST}/users/get-user?UserToken=${takeToken}`)
  ...
  useEffect(() => {
    const findSeller = async () => {
      const data = await fetch(`${IP_HOST}/users/get-seller?SellerToken=${productId.sellerToken}`)
      const body = await data.json()
  ...
  var handleClick = async () => {
    ...
    const dataOrder = await fetch(`${IP_HOST}/orders/validate-order`, {
      method: 'POST',
